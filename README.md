<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/julius-berzinskas10)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:berzinskas.julius@gmail.com)

</div>

---

##  About Me

A developer based in **Lithuania**, currently finishing my Bachelor's degree in Programming and Internet Technologies at **Vilnius Business College**.

I build **web and mobile apps** and I'm actively looking for my first dev role — frontend, backend, full-stack, or mobile. **Open to remote or on-site anywhere in Europe and Asia.**

🚀 Currently building **SolarRadar** — a full-stack solar plant maintenance system as my bachelor's thesis  
🌱 Strengthening my React, Kotlin, and backend skills  

---

## 🛠️ Tech Stack

<div align="center">

[![My Skills](https://skillicons.dev/icons?i=js,kotlin,php,java,react,html,css,laravel,firebase,mysql,github)](https://skillicons.dev)

</div>

**Languages:** JavaScript · Kotlin · PHP · Java · SQL  
**Frontend:** React · Material UI · Vite · HTML · CSS  
**Backend:** Laravel · Node.js · Firebase (BaaS)  
**Mobile:** Kotlin · Jetpack Compose  
**Databases:** Cloud Firestore · MySQL  
**Tools:** Git · GitHub · Android Studio · VS Code

---

## 🚀 Featured Projects

| Project | Description | Stack |
|---------|-------------|-------|
| 🌞 **[SolarRadar](https://solarradar.lt)** | Full-stack solar plant maintenance & fault tracking system — bachelor's thesis (currently private DEMO available) | React, Kotlin, Firebase |
| 🎓 **[Laravel Conference Manager](https://github.com/juliusberzinskas/laravel-conference-manager)** | Multi-role conference platform with admin/employee/client subsystems | Laravel 12, PHP, MySQL |
| 🏃 **[Fitness Tracker](https://github.com/juliusberzinskas/Fitness-Tracker)** | Android app with GPS workouts, step counter, Firebase sync | Kotlin, Android, Firebase |
| ☀️ **[Solar Planner](https://github.com/juliusberzinskas/SolarPlanner)** | Map-based solar panel placement and area calculator | JavaScript, Google Maps |
| 📚 **[Book Club](https://github.com/juliusberzinskas/book-club-php)** | Book sharing web app with user accounts and image uploads | PHP, MySQL |

---

## 🎯 Currently

- 🎓 Finishing my Bachelor's degree at **Vilnius Business College**
- 💼 Looking for a **junior developer position** (frontend, backend, full-stack, or mobile)
- 🌍 Open to **remote work across Europe and Asia**, or relocation

---

<div align="center">

![Profile Views](https://komarev.com/ghpvc/?username=juliusberzinskas&color=0077B5&style=for-the-badge)

</div>
